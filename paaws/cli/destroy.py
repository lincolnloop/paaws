...
@destroy.command
def account():
    cprint("This will destroy the account-level resources for Paaws. Are you sure you want to continue?", "red")
    if input("type \"destroy\" to continue") != "destroy":
        exit(0)
    stack_id = json.loads(ssm.get_parameter(Name="/paaws/account")["Parameter"]["Value"])["stack_id"]
...
@click.argument("name")
def cluster(name):
    stack_id = json.loads(ssm.get_parameter(Name=f"/paaws/cluster/{name}")["Parameter"]["Value"])[
        "stack_id"]
    cprint(f"This will destroy the Paaws cluster \"{name}\". Are you sure you want to continue?", "red")
    if input("type \"destroy\" to continue") != "destroy":
        exit(0)
...
@click.argument("name")
def app(name):
    stack_id = json.loads(ssm.get_parameter(Name=f"/paaws/apps/{name}/settings")["Parameter"]["Value"])[
        "stack_id"]
    cprint(f"This will destroy the Paaws app \"{name}\". Are you sure you want to continue?", "red")
    if input("type \"destroy\" to continue") != "destroy":
        exit(0)
